...
hn_clean = []
for story in hn:
    hn_clean.append(del_key(story, 'createdAtI'))
...
# def extract_key(dict_, key):
#     # create a copy so we don't
#     # modify the original dict
#     modified_dict = dict_.copy()
#     return modified_dict[key]


# urls = [extract_key(story, 'url') for story in hn_clean]
...
def num_comments(dict_):
    return dict_['numComments']

most_comments = max(hn_clean, key=num_comments)
